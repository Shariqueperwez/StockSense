...
| GET | `/api/market/quote/{symbol}` | No | Live stock quote and fundamentals |
| GET | `/api/market/history/{symbol}` | No | Price history for charts |
| GET | `/api/market/technicals/{symbol}` | No | RSI, MACD, BB, SMA, Volume |
| GET | `/api/market/options/{symbol}` | No | PCR, max pain, call/put volumes |
| GET | `/api/market/movers` | No | Top gainers and losers |
| GET | `/api/market/heatmap` | No | Nifty 50 sector heatmap data |
| GET | `/api/market/news/{symbol}` | No | Recent news headlines |
| GET | `/api/portfolio/` | Yes | Get user portfolio holdings |
| POST | `/api/portfolio/buy` | Yes | Buy stock at live price |
| POST | `/api/portfolio/sell` | Yes | Sell stock at live price |
| GET | `/api/portfolio/transactions` | Yes | Full transaction history |
| GET | `/api/portfolio/stress-test` | Yes | Portfolio risk and scenario analysis |
| GET | `/api/ai/thesis/{symbol}` | No | AI trader or investor thesis |
| POST | `/api/ai/chat` | Yes | AI copilot conversation |
| GET | `/api/ai/allocation` | No | AI portfolio allocation advice |
| GET | `/api/watchlist/` | Yes | Get watchlist |
| POST | `/api/watchlist/add` | Yes | Add stock to watchlist |
| DELETE | `/api/watchlist/remove/{symbol}` | Yes | Remove from watchlist |
